    ...
    def __getitem__(self, idx):
        ...
        if not self.use_h5:
            ...
            if self.modal == 'coattn':
                ...
                for slide_id in slide_ids:
                    try:
                        ...
                        with h5py.File(wsi_path, 'r') as hdf5_file:
                            ...
                            features = rearrange(features, 'b p1 p2 c -> (b p1 p2) c')
                            features2 = hdf5_file['features2'][:]
                            coords1 = hdf5_file['coords'][:]
                            # Vertical
                            features3 = hdf5_file['features3'][:]
                            features3 = rearrange(features3, 'b p1 c -> (b p1) c')
                            features4 = hdf5_file['features4'][:]
                            coords2 = hdf5_file['coords_vec'][:]
                            # Left oblique
                            features5 = hdf5_file['features5'][:]
                            features5 = rearrange(features5, 'b p1 c -> (b p1) c')
                            features6 = hdf5_file['features6'][:]
                            coords3 = hdf5_file['coord_topleft'][:]
                            # Right oblique
                            features7 = hdf5_file['features7'][:]
                            features7 = rearrange(features7, 'b p1 c -> (b p1) c')
                            features8 = hdf5_file['features8'][:]
                            coords4 = hdf5_file['coord_topright'][:]
                            # Loopback
                            features9 = hdf5_file['features9'][:]
                            features9 = rearrange(features9, 'b p1 c -> (b p1) c')
                            features10 = hdf5_file['features10'][:]
                            coords5 = hdf5_file['coord_hui'][:]

                        features1 = torch.from_numpy(features)
                        features2 = torch.from_numpy(features2)
                        features3 = torch.from_numpy(features3)
                        features4 = torch.from_numpy(features4)
                        features5 = torch.from_numpy(features5)
                        features6 = torch.from_numpy(features6)
                        features7 = torch.from_numpy(features7)
                        features8 = torch.from_numpy(features8)
                        features9 = torch.from_numpy(features9)
                        features10 = torch.from_numpy(features10)
                        coords1 = torch.from_numpy(coords1)
                        coords2 = torch.from_numpy(coords2)
                        coords3 = torch.from_numpy(coords3)
                        coords4 = torch.from_numpy(coords4)
                        coords5 = torch.from_numpy(coords5)


                        path_features2.append(features2)
                        path_features1.append(features1)
                        path_features4.append(features4)
                        path_features3.append(features3)
                        path_features6.append(features6)
                        path_features5.append(features5)
                        path_features8.append(features8)
                        path_features7.append(features7)
                        path_features10.append(features10)
                        path_features9.append(features9)
                        path_coord1.append(coords1)
                        path_coord2.append(coords2)
                        path_coord3.append(coords3)
                        path_coord4.append(coords4)
                        path_coord5.append(coords5)
                        index_num.append(torch.tensor(len(coords1)))
                    except FileNotFoundError:
                        print('FileNotFound: ', wsi_path)
                        continue
                if len(path_features1) > 0:
                    path_features1 = torch.cat(path_features1, dim=0)
                    path_features2 = torch.cat(path_features2, dim=0)
                    path_features3 = torch.cat(path_features3, dim=0)
                    path_features4 = torch.cat(path_features4, dim=0)
                    path_features5 = torch.cat(path_features5, dim=0)
                    path_features6 = torch.cat(path_features6, dim=0)
                    path_features7 = torch.cat(path_features7, dim=0)
                    path_features8 = torch.cat(path_features8, dim=0)
                    path_features9 = torch.cat(path_features9, dim=0)
                    path_features10 = torch.cat(path_features10, dim=0)
                    path_coord1 = torch.cat(path_coord1, dim=0)
                    path_coord2 = torch.cat(path_coord2, dim=0)
                    path_coord3 = torch.cat(path_coord3, dim=0)
                    path_coord4 = torch.cat(path_coord4, dim=0)
                    path_coord5 = torch.cat(path_coord5, dim=0)

                else:
                    path_features1 = torch.randn(366,1024)
                    path_features2 = torch.randn(366,1024)
                    path_features3 = torch.randn(366,1024)
                    path_features4 = torch.randn(366,1024)
                    path_features5 = torch.randn(366,1024)
                    path_features6 = torch.randn(366,1024)
                    path_features7 = torch.randn(366,1024)
                    path_features8 = torch.randn(366,1024)
                    path_features9 = torch.randn(366,1024)
                    path_features10 = torch.randn(366,1024)
                ...
                return (path_features1, path_features2, path_features3, path_features4, path_features5, path_features6, path_features7, path_features8, path_features9, path_features10, path_coord1, path_coord2, path_coord3, path_coord4, path_coord5, index_num, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c)
