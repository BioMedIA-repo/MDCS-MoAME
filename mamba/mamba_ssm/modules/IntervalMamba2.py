...
def mapping_index(delta, K):
    interval_index = torch.zeros(K, dtype=torch.long)
    reorder_index = torch.zeros(K, dtype=torch.long)
    for i in range(0, K):
        new_pos = math.ceil((i+1) * delta / K) + (i % (K // delta)) * delta - 1
        interval_index[i] = new_pos
    for i, idx in enumerate(interval_index):
        reorder_index[idx] = i
    return interval_index,reorder_index

class MDCS_interval_scanning:
    @staticmethod
    def interval_scanning(x, delta):
        # input x: x | z
        B, N, C = x.shape
        value = N // delta
        if N % delta != 0:
            padding_length = (value + 1) * delta - N
            padded_x = torch.nn.functional.pad(x, (0, 0, 0, padding_length))
        else:
            padded_x = x
        interval_index , _ = mapping_index(delta, N+padding_length)
        x_ = padded_x[:, interval_index, :]
        return x_

    @staticmethod
    def interval_reorder(x, delta, length):
        B, N, C = x.shape
        _, reorder_index = mapping_index(delta, N)
        x = x[:, reorder_index, :]
        x = x[:,:length,:]
        return x

class IntervalMamba2(nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        d_model,
        d_state=128,
    ...
    def forward(self, u, seqlen=None, seq_idx=None, cu_seqlens=None, inference_params=None):
        ...
        if self.use_mem_eff_path and inference_params is None:
            ...
            N, L, C = zxbcdt.shape
            zxbcdt_b = MDCS_interval_scanning.interval_scanning(zxbcdt, delta=5)
            ...
            out_b = MDCS_interval_scanning.interval_reorder(out_b, delta=5, length=L)
