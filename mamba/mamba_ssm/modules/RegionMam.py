...
class TransposeTokenReEmbedding:
    @staticmethod
    def transpose_region_vec(x, coords, add_length):
        ...
        for num in range(len(coords[0])):
            ...
            B, N, C = truncated_feature.shape
            corrected_truncated_feature = torch.randn(B, N, C).to('cuda')
            for i in range(len(coords[0][num])):
                index = ( coords[1][num] == coords[0][num][i]).all(dim=1).nonzero(as_tuple=True)[0]
    ...
    def transpose_region_left_oblique(x, coords, add_length):
        B, N, C = x.shape
        x1 = x[:, :N - add_length, :]
        x2 = x[:, N - add_length:, :]
        left_index = 0
    ...
    def transpose_region_right_oblique(x, coords, add_length):
        B, N, C = x.shape
        x1 = x[:, :N - add_length, :]
        x2 = x[:, N - add_length:, :]
        left_index = 0
    ...
    def transpose_region_loopback(x, coords, add_length):
        B, N, C = x.shape
        x1 = x[:, :N - add_length, :]
        x2 = x[:, N - add_length:, :]
        left_index = 0
...
class RegionMam(nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        d_model,
        d_state=128,
    ...
    def forward(self, u,u2,u3,u4,u5,coords,add_length, seqlen=None, seq_idx=None, cu_seqlens=None, inference_params=None):
        ...
        if inference_params is not None: # 不进
        ...
        if self.use_mem_eff_path and inference_params is None:
            ...
            out2 = TransposeTokenReEmbedding.transpose_region_vec(out2, coords, add_length)
            out3 = TransposeTokenReEmbedding.transpose_region_left_oblique(out3, coords, add_length)
            out4 = TransposeTokenReEmbedding.transpose_region_right_oblique(out4, coords, add_length)
            out5 = TransposeTokenReEmbedding.transpose_region_loopback(out5, coords, add_length)

            out = out + out2 + out3 + out4 + out5
